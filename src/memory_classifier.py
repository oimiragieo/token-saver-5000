...
_CATEGORY_PATTERNS = {
    "gotcha": [
        r"\bwatch\s*out\b", r"\bgotcha\b", r"\bcaveat\b", r"\bpitfall\b",
        r"\btrap\b", r"\bcareful\b", r"\bsilently\b", r"\bnever\b.*\buse\b",
        r"\bwarning\b", r"\bbeware\b", r"\bgotchas?\b", r"\bavoid\b",
        r"\bdon'?t\b.*\bforget\b", r"\bsubtl[ey]\b",
    ],
    "issue": [
        r"\bbug\b", r"\berror\b", r"\bfail(?:s|ed|ure|ing)?\b", r"\bcrash\b",
        r"\bbroken\b", r"\bregression\b", r"\btimeout\b", r"\bnull\s*pointer\b",
        r"\bexception\b", r"\bflaw\b", r"\bdefect\b",
    ],
    "decision": [
        r"\bdecid(?:e|ed)\b", r"\bchose\b", r"\bchoice\b", r"\binstead\s*of\b",
        r"\bopt(?:ed)?\s*for\b", r"\bwe\s*went\s*with\b", r"\breason(?:ing)?\s*(?:for|behind)\b",
        r"\btrade-?off\b", r"\bselect(?:ed|ion)\b",
    ],
    "pattern": [
        r"\bpattern\b", r"\bconsistently\b", r"\balways\b.*\buse\b",
        r"\bconvention\b", r"\bbest\s*practice\b", r"\bidiom\b",
        r"\brecurring\b", r"\bstandard\b.*\bapproach\b",
    ],
}
