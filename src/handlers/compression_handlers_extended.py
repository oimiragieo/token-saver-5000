...
async def handle_compress_codebase(context: HandlerContext, args: Dict[str, Any]) -> str:
    ...
    # context". That is an invariant held by ONE set of callers, not a property
    # of this function -- exactly the shape that breaks when the next caller
    # appears without knowing the invariant exists. Measured: only two sites
    # construct this context (`context_service.py`, `server_factory_service.py`)
    # and BOTH inject the validator, so failing closed cannot break a real
    # server path -- it can only refuse a context that was never entitled to
    # skip confinement.
    #
